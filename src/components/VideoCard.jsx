...
const VideoCard = ({ video }) => {
    ...
    const fetchVideoData = async () => {
      try {
        const response = await API.get(
          `api/video/check-like/${video._id}/${user._id}`
        );
  ...
  const handleLike = async () => {
    ...
    try {
      if (!liked) {
        // Like the video
        const response = await API.get(
          `api/video/like/${video._id}/${user._id}`
        );
        ...
        // Unlike the video
        const response = await API.get(
          `api/video/unlike/${video._id}/${user._id}`
        );
        setresVideo((prev) => ({ ...prev, likes: response.data.likes })); // Update likes
        setLiked(false);
      }
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const response = await API.post(`api/video/comment/${video._id}`, {
        comment,
        user,
      });
  ...
  const fetchComments = async () => {
    try {
      const response = await API.get(`api/video/comment/${video._id}`);
